...
public class ReportService {
  ...
  public Map<String, Object> getReviewResultsForKid(UUID sessionId, UUID kidId) {
    ...
    if (!session.isCompleted()) {
      throw new InvalidOperationException(
          "Review results are only available for completed sessions.");
    }
            ...
            .orElseThrow(
                () ->
                    new InvalidOperationException(
                        "Kid with ID " + kidId + " did not participate in this session."));
    if (participant.isAbsent()) {
      throw new InvalidOperationException("Absent kids are not eligible for review results.");
    }

    List<InstructorAnswer> instructorAnswers =
        instructorAnswerRepository.findBySessionAndKid(session, kid);
    ...
    Map<String, Integer> instructorReview =
        instructorAnswers.stream()
            .collect(
                Collectors.toMap(ans -> ans.getSoftSkill().getName(), InstructorAnswer::getAnswer));
    results.put("instructorReview", instructorReview);

    Map<String, Integer> kidSelfReview =
        kidAnswers.stream()
            .collect(Collectors.toMap(ans -> ans.getSoftSkill().getName(), KidAnswer::getAnswer));
    ...
    for (SoftSkill softSkill : session.getSoftSkills()) {
      ...
      if (instAnswer != null && kidAnswer != null) {
        if (instAnswer.equals(kidAnswer)) {
          ...
          comparison.put(
              softSkill.getName(),
              "Instructor rated higher (Difference: " + (instAnswer - kidAnswer) + ")");
        } else {
          comparison.put(
              softSkill.getName(),
              "Kid rated higher (Difference: " + (kidAnswer - instAnswer) + ")");
        }
      } else {
  ...
  public Map<String, Object> getSessionSummary(UUID sessionId) {
    ...
    summary.put(
        "targetedSoftSkills",
        softSkills.stream().map(SoftSkill::getName).collect(Collectors.toList()));
    ...
    long totalKids =
        allParticipants.stream().filter(p -> p.getUser().getUserType() == UserType.KID).count();
    long totalInstructors =
        allParticipants.stream()
            .filter(p -> p.getUser().getUserType() == UserType.INSTRUCTOR)
            .count();
    long absentKids =
        allParticipants.stream()
            .filter(p -> p.getUser().getUserType() == UserType.KID && p.isAbsent())
            .count();
    ...
    if (session.isCompleted()) {
      ...
      for (SoftSkill softSkill : softSkills) {
        double avgInst =
            instructorAnswerRepository
                .findBySessionAndKid(
                    session, null) // Needs a custom query for all kids for a soft skill
                .stream()
